...
/*
 struct tag
 {
	...
	uint8_t ctx_specific:1;
	uint8_t len_value_type:3;
 }bacnet_tag_t;
 */
...
)
{
    ...
    len = decode_tag_number_safe(&apdu[0], 
								 apdu_len_remaining, 
								 tag_number);
...
)
{
    ...
    len = encode_tag(&apdu[0], 
					 ...
					 true, 
					 4);
    len += encode_bacnet_object_id(&apdu[len], 
								   object_type, 
								   instance);
...
)
{
    ...
    if (decode_is_context_tag(&apdu[len], tag_number))
    {
        ...
        status = octetstring_init(octet_string, 
								  &apdu[len], 
								  len_value);
