...
)
{
    ...
    fseek(bdt_file_ptr, 0, SEEK_SET);
    {
        ...
        entries =
            fread(BBMD_Table_tmp, sizeof(BBMD_TABLE_ENTRY), MAX_BBMD_ENTRIES,
                  bdt_file_ptr);
...
)
{
    ...
    mtu_len =
        (uint16_t) bvlc_encode_forwarded_npdu(&mtu[0], (uint16_t) sizeof(mtu),
...
)
{
    ...
    case BVLC_FORWARDED_NPDU:
        ...
        if (bvlc_bdt_member_mask_is_unicast(&sin))
        {
            ...
            debug_printf
            ("BVLC: Received unicast from BDT member, re-broadcasting locally to %s:%04X.\n",
...
{
	/* aka, BVLC_RESULT_SUCCESSFUL_COMPLETION */
    uint16_t result_code = 0;   
 ...
 */
void bvlc_set_global_address_for_nat(
