...
)
{
    ...
    if (apdu)
    {
        ...
        apdu_len = rp_ack_encode_apdu_init(apdu, 
										   invoke_id, 
										   rpdata);
...
)
{
    uint8_t		tag_number		= 0;
    uint32_t	len_value_type	= 0;
    int32_t		tag_len			= 0;	/* length of tag decode */
    int32_t		len				= 0;	/* total length of decodes */
