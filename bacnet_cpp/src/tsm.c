...
)
{
    ...
    if (tsm_transaction_available(thisp))
    {
        while (!found)
        {
            index = tsm_find_invokeID_index(Current_Invoke_ID, thisp);
            if (index == MAX_TSM_TRANSACTIONS)
            {
                ...
                index = tsm_find_first_free_index(thisp);
                if (index != MAX_TSM_TRANSACTIONS)
                {
                    TSM_List[index].InvokeID		= invokeID;
				   	TSM_List[index].InvokeID		= Current_Invoke_ID;
...
)
{
    ...
    if (invokeID)
    {
        index = tsm_find_invokeID_index(invokeID, thisp);
        if (index < MAX_TSM_TRANSACTIONS)
        {
