...
)
{
    ...
    if (apdu)
    {
        ...
        apdu_len += encode_context_enumerated(&apdu[apdu_len], 
											  1, 
        ...
        {
            apdu_len += encode_context_unsigned(&apdu[apdu_len], 
												2, 
												rrdata->array_index);
        }
...
)
{
    ...
    if (apdu_len && rrdata)
    {
		...
		/* Start with the fixed overhead */
        rrdata->Overhead = RR_OVERHEAD; 

        /* Tag 2: Optional Array Index - set to ALL if not present */
		/* Assuming this is the most common outcome... */
        rrdata->array_index = BACNET_ARRAY_ALL; 
        if (len < apdu_len)
        {
            TagLen = (unsigned) decode_tag_number_and_value(&apdu[len], 
															&tag_number,
															&len_value_type);
            if (tag_number == 2)
            {
                ...
                rrdata->Overhead += RR_INDEX_OVERHEAD;  
            }
        }

        /* And/or optional range selection- Tags 3, 6 and 7 */
		/* Assume the worst to cut out explicit checking later */
        ...
        if (len < apdu_len)
        {
            ...
            default: /* If we don't recognise the tag then we 
					  *	do nothing here and try to return
...
)
{
    ...
    if (apdu)
    {
        ...
        /* service ack follows */
        apdu_len +=
            encode_context_object_id(&apdu[apdu_len], 
									 0, 
									 rrdata->object_type,
                                     rrdata->object_instance);
        apdu_len +=
            encode_context_enumerated(&apdu[apdu_len], 
									  1,
                                      rrdata->object_property);
...
)
{
    ...
    {
		/* a tag number of 5 is not extended so only one octet */
        ...
        while (len < apdu_len)
        {
			...
			&& (decode_is_closing_tag_number(&apdu[len], 5)))
            {
				...
				/* Step over single byte closing tag */
                len++;  
                break;
            }
            else
            {
				...
				/* Skip over data value as well */
                len += len_value_type;  

				/* APDU is exhausted so we have failed to find closing tag */
                if (len >= apdu_len)    
				{
                    return (-1);
				}
	...
	/* There should be nothing left to see here */
    len = apdu_len;     
