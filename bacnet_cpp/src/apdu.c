...
)
{
    ...
    if (dcc_communication_disabled(thisp))
    {
        switch (service_choice)
        {
        case SERVICE_CONFIRMED_DEVICE_COMMUNICATION_CONTROL:
...
)
{
    ...
    else if (dcc_communication_initiation_disabled(thisp))
    {
...
)
{
    ...
    if (apdu)
    {
        ...
        case PDU_TYPE_CONFIRMED_SERVICE_REQUEST:
            ...
            if (apdu_confirmed_dcc_disabled(service_choice, thisp))
            {
                /* When network communications are completely disabled,
                   only DeviceCommunicationControl and ReinitializeDevice APDUs
                   shall be processed and no messages shall be initiated. */
        ...
        case PDU_TYPE_UNCONFIRMED_SERVICE_REQUEST:
            ...
            if (apdu_unconfirmed_dcc_disabled(service_choice, thisp))
            {
        ...
        case PDU_TYPE_SIMPLE_ACK:
            ...
            case SERVICE_CONFIRMED_REQUEST_KEY:
                ...
                tsm_free_invoke_id(invoke_id, thisp);
                break;
            default:
                break;
            }
        ...
        case PDU_TYPE_COMPLEX_ACK:
            service_ack_data.segmented_message		= (apdu[0] & BIT(3)) ? true : false;
            ...
            case SERVICE_CONFIRMED_AUTHENTICATE:
                if (Confirmed_ACK_Function[service_choice] != NULL)
                {
                    (Confirmed_ACK_Function[service_choice])(service_request, 
															 service_request_len, 
															 src,
															 &service_ack_data,
															 thisp);
