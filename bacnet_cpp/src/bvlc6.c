...
)
{
    ...
    {
        bytes_encoded =
            bvlc6_encode_header(pdu, pdu_size,
                                BVLC6_RESULT, length);
        if (bytes_encoded == 4)
        {
...
)
{
    ...
    test_len +=
        bvlc6_decode_result(&pdu[4], length - 4,
                            &test_vmac,
                            &test_result_code);
...
)
{
    ...
    uint16_t i = 0;
    len =
        bvlc6_encode_original_unicast(pdu, sizeof(pdu),
                                      vmac_src, vmac_dst,
                                      npdu, npdu_len);
    msg_len = 10 + npdu_len;
    ...
    test_len +=
        bvlc6_decode_original_unicast(&pdu[4], length - 4,
                                      &test_vmac_src,
                                      &test_vmac_dst,
...
)
{
    ...
    uint16_t i = 0;
    len =
        bvlc6_encode_original_broadcast(pdu, sizeof(pdu),
                                        vmac, npdu,
                                        npdu_len);
    msg_len = 7 + npdu_len;
    ct_test(pTest, len == msg_len);
    ...
    ct_test(pTest, length == msg_len);
    test_len +=
        bvlc6_decode_original_broadcast(&pdu[4], length - 4,
                                        &test_vmac,
                                        test_npdu, sizeof(test_npdu), &test_npdu_len);
    ct_test(pTest, len == test_len);
    ct_test(pTest, msg_len == test_len);
...
)
{
    ...
    len =
        bvlc6_encode_address_resolution(pdu, sizeof(pdu),
                                        vmac_src,
                                        vmac_target);
    ct_test(pTest, len == msg_len);
    test_len = test_BVLC6_Header(pTest,
    ...
    ct_test(pTest, length == msg_len);
    test_len +=
        bvlc6_decode_address_resolution(&pdu[4], length - 4,
                                        &test_vmac_src,
                                        &test_vmac_target);
    ct_test(pTest, len == test_len);
    ct_test(pTest, msg_len == test_len);
...
{
    BACNET_IP6_ADDRESS test_bip6_address = { {
            0
        }
    };
    uint8_t pdu[60] =
    ...
    len =
        bvlc6_encode_forwarded_address_resolution(
            pdu, sizeof(pdu),
            vmac_src,
            vmac_dst,
            bip6_address);
    ...
    ct_test(pTest, length == msg_len);
    test_len +=
        bvlc6_decode_forwarded_address_resolution(
            &pdu[4], length - 4,
...
)
{
    ...
    const int msg_len = 10;
    len =
        bvlc6_encode_address_resolution_ack(pdu, sizeof(pdu),
                                            vmac_src,
                                            vmac_dst);
    ct_test(pTest, len == msg_len);
    test_len = test_BVLC6_Header(pTest,
    ...
    ct_test(pTest, length == msg_len);
    test_len +=
        bvlc6_decode_address_resolution_ack(&pdu[4], length - 4,
                                            &test_vmac_src, &test_vmac_dst);
    ct_test(pTest, len == test_len);
    ct_test(pTest, msg_len == test_len);
    ct_test(pTest, vmac_src == test_vmac_src);
...
)
{
    ...
    const int msg_len = 7;
    len =
        bvlc6_encode_virtual_address_resolution(pdu, sizeof(pdu),
                vmac_src);
    ct_test(pTest, len == msg_len);
    test_len = test_BVLC6_Header(pTest,
                                 pdu, len, &message_type, &length);
    ...
    ct_test(pTest, length == msg_len);
    test_len +=
        bvlc6_decode_virtual_address_resolution(&pdu[4], length - 4,
                &test_vmac_src);
    ct_test(pTest, len == test_len);
    ct_test(pTest, msg_len == test_len);
    ct_test(pTest, vmac_src == test_vmac_src);
...
)
{
    ...
    const int msg_len = 10;
    len =
        bvlc6_encode_virtual_address_resolution_ack(pdu, sizeof(pdu),
                vmac_src, vmac_dst);
    ct_test(pTest, len == msg_len);
    test_len = test_BVLC6_Header(pTest,
                                 pdu, len, &message_type, &length);
    ...
    ct_test(pTest, length == msg_len);
    test_len +=
        bvlc6_decode_virtual_address_resolution_ack(&pdu[4], length - 4,
                &test_vmac_src, &test_vmac_dst);
    ct_test(pTest, len == test_len);
    ct_test(pTest, msg_len == test_len);
    ct_test(pTest, vmac_src == test_vmac_src);
...
)
{
    ...
    uint16_t i = 0;
    len =
        bvlc6_encode_forwarded_npdu(pdu, sizeof(pdu),
    ...
    ct_test(pTest, length == msg_len);
    test_len +=
        bvlc6_decode_forwarded_npdu(&pdu[4], length - 4,
                                    &test_vmac_src,
                                    &test_bip6_address,
                                    test_npdu, sizeof(test_npdu), &test_npdu_len);
    ct_test(pTest, len == test_len);
...
)
{
    ...
    const int msg_len = 9;
    len =
        bvlc6_encode_register_foreign_device(pdu, sizeof(pdu),
                vmac_src,
                ttl_seconds);
    ct_test(pTest, len == msg_len);
    test_len = test_BVLC6_Header(pTest,
    ...
    ct_test(pTest, length == msg_len);
    test_len +=
        bvlc6_decode_register_foreign_device(&pdu[4], length - 4,
...
)
{
    ...
    uint32_t test_vmac_src = 0;
    BACNET_IP6_FOREIGN_DEVICE_TABLE_ENTRY test_fdt_entry =
    {
        0
    };
    uint8_t message_type = 0;
    ...
    len =
        bvlc6_encode_delete_foreign_device(pdu, sizeof(pdu),
                                           vmac_src,
                                           fdt_entry);
    ct_test(pTest, len == msg_len);
    test_len = test_BVLC6_Header(pTest,
    ...
    ct_test(pTest, length == msg_len);
    test_len +=
        bvlc6_decode_delete_foreign_device(&pdu[4], length - 4,
...
)
{
    ...
    ct_test(pTest, length == msg_len);
    test_len +=
        bvlc6_decode_secure_bvll(&pdu[4], length - 4,
                                 test_sbuf,
...
)
{
    ...
    uint16_t i = 0;
    len =
        bvlc6_encode_distribute_broadcast_to_network(pdu, sizeof(pdu),
                vmac,
                npdu, npdu_len);
    msg_len = 7 + npdu_len;
    ct_test(pTest, len == msg_len);
    ...
    ct_test(pTest, length == msg_len);
    test_len +=
        bvlc6_decode_distribute_broadcast_to_network(
            &pdu[4], length - 4,
...
)
{
    ...
    for (i = 0; i < 16; i++)
    {
        ...
        ct_test(pTest, status);
        status =
            bvlc6_address_get(&src,
                              &test_group, NULL, NULL, NULL, NULL, NULL,
                              NULL, NULL);
        ct_test(pTest, status);
...
)
{
    ...
    assert(rc);
    rc =
        ct_addTestFunction(pTest, test_BVLC6_Distribute_Broadcast_To_Network);
    assert(rc);
