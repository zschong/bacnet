...
void address_protected_entry_index_set(
    uint32_t top_protected_entry_index
)
{
    Top_Protected_Entry = top_protected_entry_index;
}

void address_own_device_id_set(
    uint32_t own_id
)
{
...
)
{
    ...
    while (pMatch <= &Address_Cache[MAX_ADDRESS_CACHE - 1])
    {
        if ((pMatch->Flags & (BAC_ADDR_IN_USE | BAC_ADDR_BIND_REQ)) == BAC_ADDR_IN_USE)
        {
            iLen += encode_application_object_id(&apdu[iLen], 
												OBJECT_DEVICE,
												pMatch->device_id);

            iLen += encode_application_unsigned(&apdu[iLen], 
												pMatch->address.net);
            ...
            {
                octetstring_init(&MAC_Address, 
								 pMatch->address.adr,
                                 pMatch->address.len);

                iLen += encode_application_octet_string(&apdu[iLen], 
														&MAC_Address);
            }
            else
            {
...
)
{
	...
	/* Find first bound entry */
    while ((pMatch->Flags & (BAC_ADDR_IN_USE | BAC_ADDR_BIND_REQ)) != BAC_ADDR_IN_USE)
	{
        pMatch++;
	}
    ...
    while (uiIndex <= uiTarget)
    {
        if (uiRemaining < ACACHE_MAX_ENC)
        {
             ...
             */
            bitstring_set_bit(&pRequest->ResultFlags, 
...
)
{
    ...
    src.adr[0]	= 0;
    max_apdu	= 50;
    set_file_address(Address_Cache_Filename, 
					 device_id, 
					 &src, 
					 max_apdu);
    ...
    address_file_init(Address_Cache_Filename);
    ct_test(pTest, address_get_by_device(device_id, 
										 &test_max_apdu,
                                         &test_address));
...
)
{
    ...
    for (i = 0; i < MAX_ADDRESS_CACHE; i++)
    {
        ...
        ct_test(pTest, test_max_apdu == max_apdu);
        ct_test(pTest, bacnet_address_same(&test_address, &src));
        ct_test(pTest, address_get_by_index(i, 
											&test_device_id, 
											&test_max_apdu,
                                            &test_address));
