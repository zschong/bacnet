...
)
{
    ...
    if ((dest->net == BACNET_BROADCAST_NETWORK) || (dest->mac_len == 0))
    {
        ...
        if (Remote_BBMD.port)
        {
            ...
            mtu_len =
                bvlc6_encode_distribute_broadcast_to_network(mtu, sizeof(mtu),
        ...
        else
        {
            ...
            mtu_len =
                bvlc6_encode_original_broadcast(mtu, sizeof(mtu), vmac_src,
                                                pdu, pdu_len);
    ...
    else if ((dest->net > 0) && (dest->len == 0))
    {
        ...
        mtu_len =
            bvlc6_encode_original_broadcast(mtu, sizeof(mtu), vmac_src, pdu,
                                            pdu_len);
    ...
    else if (dest->mac_len == 3)
    {
        ...
        mtu_len =
            bvlc6_encode_original_unicast(mtu, sizeof(mtu), vmac_src, vmac_dst,
                                          pdu, pdu_len);
...
)
{
    ...
    mtu_len =
        bvlc6_encode_address_resolution_ack(&mtu[0], sizeof(mtu), vmac_src,
                                            vmac_dst);
...
)
{
    ...
    mtu_len =
        bvlc6_encode_virtual_address_resolution_ack(&mtu[0], sizeof(mtu),
                vmac_src, vmac_dst);
...
)
{
    ...
    if (addr && pdu)
    {
        ...
        else
        {
            ...
            function_len =
                bvlc6_decode_virtual_address_resolution(pdu, pdu_len,
                        &vmac_src);
...
)
{
    ...
    if (addr && pdu)
    {
        ...
        else
        {
            ...
            function_len =
                bvlc6_decode_virtual_address_resolution_ack(pdu, pdu_len,
                        &vmac_src, &vmac_dst);
...
)
{
    ...
    if (addr && pdu)
    {
        ...
        else
        {
            ...
            function_len =
                bvlc6_decode_address_resolution(pdu, pdu_len, &vmac_src,
                                                &vmac_target);
...
)
{
    ...
    if (addr && pdu)
    {
        ...
        else
        {
            ...
            function_len =
                bvlc6_decode_address_resolution_ack(pdu, pdu_len, &vmac_src,
                                                    &vmac_dst);
...
)
{
    ...
    BACNET_IP6_ADDRESS fwd_address = { {
            0
        }
    };
    ...
    if (header_len == 4)
    {
        ...
        case BVLC6_ORIGINAL_UNICAST_NPDU:
            ...
            else
            {
                ...
                function_len =
                    bvlc6_decode_original_unicast(pdu, pdu_len, &vmac_src,
                                                  &vmac_dst, NULL, 0, &npdu_len);
        ...
        case BVLC6_ORIGINAL_BROADCAST_NPDU:
            ...
            function_len =
                bvlc6_decode_original_broadcast(pdu, pdu_len, &vmac_src,
                                                NULL, 0, &npdu_len);
            ...
            if (function_len)
            {
                ...
                /*  Upon receipt of a BVLL Original-Broadcast-NPDU
                   ...
                   the BBMD's FDT */
                BVLC6_Buffer_Len =
                    bvlc6_encode_forwarded_npdu(&BVLC6_Buffer[0],
                                                sizeof(BVLC6_Buffer), vmac_src, addr, npdu, npdu_len);
        ...
        case BVLC6_FORWARDED_NPDU:
            ...
            function_len =
                bvlc6_decode_forwarded_npdu(pdu, pdu_len, &vmac_src,
                                            &fwd_address, NULL, 0, &npdu_len);
    ...
    if (header_len == 4)
    {
        ...
        case BVLC6_ORIGINAL_BROADCAST_NPDU:
            ...
            else
            {
                ...
                function_len =
                    bvlc6_decode_original_broadcast(pdu, pdu_len,
                                                    &vmac_src, NULL, 0, &npdu_len);
...
)
{
    ...
    mtu_len =
        bvlc6_encode_register_foreign_device(&mtu[0], sizeof(mtu), vmac_src,
...
)
{
    ...
    for (i = 0; i < 6; i++)
    {
        ...
        mtu_len =
            bvlc6_encode_result(&mtu[0], sizeof(mtu), vmac_src,
                                result_code[i]);
